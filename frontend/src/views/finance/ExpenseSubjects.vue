<template>
  <div class="page-container">
    <el-card>
      <template #header>
        <div class="card-header"><span>支出科目</span><el-button type="primary" size="small" @click="dialogVisible = true">新增</el-button></div>
      </template>
      <el-table :data="tableData" stripe border size="small" row-key="id" default-expand-all>
        <el-table-column prop="name" label="科目名称" min-width="200" />
        <el-table-column prop="parent" label="所属父级" width="120" />
    ...
    <el-dialog v-model="dialogVisible" title="新增支出科目" width="450px">
      <el-form :model="form" label-width="100px" ref="formRef">
        <el-form-item label="科目名称" prop="name"><el-input v-model="form.name" /></el-form-item>
        <el-form-item label="所属父级">
          <el-cascader v-model="form.parent_id" :options="parentOptions" :props="{ checkStrictly: true, value: 'id', label: 'name' }" clearable style="width:100%" />
        </el-form-item>
...
const form = reactive({ name: '', parent_id: [], status: 'active', remark: '' })
const parentOptions = tableData
onMounted(async () => {
  const res = await financeApi.getExpenseSubjects()
